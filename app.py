...
st.write("Step 1 :Paste a News article in the input textbook");
st.write("Step 2 :Select the classifier and wait for the results :) ");
...
if len(text_input)> 0:
    ...
    if model.predict_proba(text)[0][1]>0.5:
        st.markdown("<h3 style='text-align: center; color: #000000;'>{}</h3>".format("Stock will rise"), unsafe_allow_html=True)
    else: 
        st.markdown("<h3 style='text-align: center; color: #000000;'>{}</h3>".format("Stock will fall"), unsafe_allow_html=True)
